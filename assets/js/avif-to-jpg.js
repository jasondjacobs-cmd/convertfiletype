(() => {
  ...
  const selectFile = (file) => {
    ...
    if (avifSupported === false) {
      reset();
      status.textContent = 'This browser cannot decode AVIF images. Try a current version of Chrome, Edge, Firefox, or Safari.';
      return;
    }
    if (!isAvif(file)) {
      reset();
  ...
  convertButton.addEventListener('click', async () => {
    ...
    try {
      const conversionPromise = (async () => {
        ...
        const bytes = new Uint8Array(await jpgBlob.slice(0, 3).arrayBuffer());
        if (bytes[0] !== 0xff || bytes[1] !== 0xd8 || bytes[2] !== 0xff) throw new Error('Generated JPG signature is invalid.');
        ...
        if (validated.naturalWidth !== image.naturalWidth || validated.naturalHeight !== image.naturalHeight) {
          URL.revokeObjectURL(candidateUrl);
          throw new Error('Generated JPG dimensions do not match the AVIF source.');
        }
