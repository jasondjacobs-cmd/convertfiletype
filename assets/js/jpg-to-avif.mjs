...
const input = document.querySelector('[data-jpg-file]');
if (input) {
  ...
  const isJpeg = (candidate) => candidate.type === 'image/jpeg' || /\.jpe?g$/i.test(candidate.name);
  const selectFile = (candidate) => {
    ...
    if (!isJpeg(candidate)) {
      reset();
      status.textContent = 'Please choose a JPG or JPEG image.';
      return;
    }
  ...
  button.addEventListener('click', async () => {
    ...
    try {
      const conversion = (async () => {
        setProgress(8, 'Reading JPG file', `${formatBytes(source.size)} source file`);
        status.textContent = 'Reading your JPG on this device…';
        const sourceUrl = URL.createObjectURL(source);
        let image;
        try {
          setProgress(22, 'Decoding JPG', elapsed());
          image = await loadImage(sourceUrl, 'JPG could not be decoded by this browser.');
        } finally {
        ...
        resultUrl = candidate;
        const base = source.name.replace(/\.jpe?g$/i, '') || 'converted-image';
