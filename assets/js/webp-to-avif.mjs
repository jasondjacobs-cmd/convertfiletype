...
if (input) {
  ...
  const isWebp = (candidate) => candidate.type === 'image/webp' || /\.webp$/i.test(candidate.name);
  ...
  const validateAvifHeader = async (blob) => {
    ...
    if (box !== 'ftyp' || !/(avif|avis)/.test(header)) throw new Error('Generated AVIF signature is invalid.');
  };
  ...
  button.addEventListener('click', async () => {
    ...
    try {
      const conversion = (async () => {
        setProgress(8, 'Reading WebP file', `${formatBytes(source.size)} source file`);
        status.textContent = 'Reading your WebP on this device…';
        const sourceUrl = URL.createObjectURL(source);
        let image;
        try {
          setProgress(22, 'Decoding WebP', elapsed());
          image = await loadImage(sourceUrl, 'WebP could not be decoded by this browser.');
        } finally {
        ...
        let validated;
        try {
          validated = await withTimeout(loadImage(candidate, 'Generated AVIF could not be decoded by this browser.'), PREVIEW_TIMEOUT_MS, 'AVIF preview timed out.');
        } catch (error) {
        ...
        if (validated.naturalWidth !== image.naturalWidth || validated.naturalHeight !== image.naturalHeight) {
          URL.revokeObjectURL(candidate);
          throw new Error('Generated AVIF dimensions do not match the WebP source.');
        }
        ...
        resultUrl = candidate;
        const base = source.name.replace(/\.webp$/i, '') || 'converted-image';
