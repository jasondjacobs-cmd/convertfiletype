(() => {
  ...
  const selectFile = (file) => {
    ...
    if (!isWebp(file)) {
      reset();
      status.textContent = 'Please choose a WebP image.';
      return;
    }
  ...
  const loadImage = (url) => new Promise((resolve, reject) => {
    ...
    image.onload = () => image.naturalWidth > 0 && image.naturalHeight > 0 ? resolve(image) : reject(new Error('WebP image has invalid dimensions.'));
    image.onerror = () => reject(new Error('WebP could not be decoded by this browser.'));
    image.src = url;
  });
  ...
  convertButton.addEventListener('click', async () => {
    ...
    try {
      const conversionPromise = (async () => {
        setProgress(10, 'Reading WebP file', `${formatBytes(sourceFile.size)} source file`);
        status.textContent = 'Reading your WebP on this device…';
        const sourceUrl = URL.createObjectURL(sourceFile);
        let image;
        try {
          setProgress(30, 'Decoding WebP', elapsed());
        ...
        const bytes = new Uint8Array(await jpgBlob.slice(0, 2).arrayBuffer());
        if (bytes[0] !== 0xff || bytes[1] !== 0xd8) throw new Error('Generated JPG signature is invalid.');
        ...
        const baseName = sourceFile.name.replace(/\.webp$/i, '') || 'converted-image';
      ...
      status.textContent = message === 'Conversion timed out.' ? 'Conversion took too long. This file could not be processed quickly enough on this device.' : `Conversion failed: ${message}`;
    } finally {
