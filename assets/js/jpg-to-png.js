(() => {
  ...
  const loadImage = (url) => new Promise((resolve, reject) => {
    ...
    image.onload = () => image.naturalWidth > 0 && image.naturalHeight > 0
      ? resolve(image)
      : reject(new Error('JPG image has invalid dimensions.'));
    image.onerror = () => reject(new Error('JPG could not be decoded by this browser.'));
    image.src = url;
  });
  ...
  convertButton.addEventListener('click', async () => {
    ...
    try {
      const conversionPromise = (async () => {
        setProgress(10, 'Reading JPG file', `${formatBytes(sourceFile.size)} source file`);
        status.textContent = 'Reading your JPG on this device…';
        const sourceUrl = URL.createObjectURL(sourceFile);
        let image;
        try {
          setProgress(30, 'Decoding JPG', elapsed());
        ...
        const baseName = sourceFile.name.replace(/\.(jpe?g)$/i, '') || 'converted-image';
