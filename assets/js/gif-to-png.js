(() => {
  ...
  const selectFile = (file) => {
    ...
    if (!isGif(file)) {
      reset();
      status.textContent = 'Please choose a GIF image.';
      return;
    }
  ...
  const loadImage = (url) => new Promise((resolve, reject) => {
    ...
    image.onload = () => image.naturalWidth > 0 && image.naturalHeight > 0
      ? resolve(image)
      : reject(new Error('GIF image has invalid dimensions.'));
  ...
  convertButton.addEventListener('click', async () => {
    ...
    try {
      const conversionPromise = (async () => {
        setProgress(10, 'Reading GIF file', `${formatBytes(sourceFile.size)} source file`);
        status.textContent = 'Reading your GIF on this device…';

        const sourceUrl = URL.createObjectURL(sourceFile);
        let image;
        try {
          setProgress(30, 'Decoding GIF', elapsed());
        ...
        if (validated.naturalWidth !== image.naturalWidth || validated.naturalHeight !== image.naturalHeight) {
          URL.revokeObjectURL(candidateUrl);
          throw new Error('Generated PNG dimensions do not match the GIF source.');
        }
        ...
        const baseName = sourceFile.name.replace(/\.gif$/i, '') || 'converted-image';
