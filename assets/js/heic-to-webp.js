(() => {
  ...
  const validateWebpSignature = async (blob) => {
    const bytes = new Uint8Array(await blob.slice(0, 12).arrayBuffer());
    const signature = new TextDecoder('latin1').decode(bytes);
    if (!signature.startsWith('RIFF') || signature.slice(8, 12) !== 'WEBP') {
      throw new Error('Generated WebP signature is invalid.');
    }
  };
  ...
  const waitForPreview = (url) => new Promise((resolve, reject) => {
    ...
    testImage.onerror = () => reject(new Error('Converted WebP could not be decoded.'));
    testImage.src = url;
  });
  ...
  convertButton.addEventListener('click', async () => {
    ...
    try {
      const conversionPromise = (async () => {
        ...
        setProgress(70, 'Encoding WebP', `${rendered.width} × ${rendered.height} · ${quality.value}% quality · ${elapsed()}`);
        const webpBlob = await canvasToWebp(rendered.canvas, Number(quality.value) / 100);
        if ((webpBlob.type || '').toLowerCase() !== 'image/webp') {
          throw new Error(`Unexpected output type: ${webpBlob.type || 'unknown'}`);
        }

        setProgress(86, 'Validating WebP', `${formatBytes(webpBlob.size)} output · ${elapsed()}`);
        await validateWebpSignature(webpBlob);
        ...
        if (dimensions.width !== rendered.width || dimensions.height !== rendered.height) {
          URL.revokeObjectURL(candidateUrl);
          throw new Error('Generated WebP dimensions do not match the HEIC source.');
        }
        ...
        resultSize.textContent = `${formatBytes(webpBlob.size)} WebP · ${dimensions.width} × ${dimensions.height}`;
