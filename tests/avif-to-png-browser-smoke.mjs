...
const fixturePath = '/tmp/avif-to-png-fixture.avif';
...
try {
  await page.goto(`${SITE_URL}/avif-to-png/`, { waitUntil: 'networkidle' });
  ...
  assert.equal(await page.locator('[data-build-version]').textContent(), 'Converter build: avif-png-v1-native');
  ...
  const output = await page.locator('[data-download]').evaluate(async (anchor) => {
    ...
    return { type: blob.type, size: blob.size, head: [...bytes.slice(0, 8)], width: image.naturalWidth, height: image.naturalHeight, corner };
  });

  assert.equal(output.type, 'image/png');
  assert.ok(output.size > 0);
  assert.deepEqual(output.head, [137, 80, 78, 71, 13, 10, 26, 10]);
  assert.equal(output.width, 160);
  assert.equal(output.height, 100);
  assert.ok(output.corner[3] <= 5, `transparent corner alpha should be preserved: ${output.corner}`);

  await page.locator('[data-convert-another]').click();
  assert.ok(await page.locator('[data-result]').isHidden());
  assert.equal(consoleErrors.length, 0, `console errors: ${consoleErrors.join(' | ')}`);
  assert.equal(pageErrors.length, 0, `page errors: ${pageErrors.join(' | ')}`);
  console.log(`AVIF to PNG browser smoke passed in ${elapsed}ms.`);
} finally {
  await browser.close();
}