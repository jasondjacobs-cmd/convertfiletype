...
try {
  await page.goto(`${siteUrl}/webp-to-avif/`, { waitUntil: 'networkidle' });
  await page.getByText('Converter build: webp-avif-v1-wasm').waitFor();
  assert.equal(await page.locator('script:not([src])').count(), 0, 'WebP to AVIF page should not use inline scripts');
  ...
  await page.locator('[data-webp-file]').setInputFiles({
    name: 'webp-to-avif-smoke.webp',
    mimeType: 'image/webp',
  ...
  assert.equal(result.downloadName, 'webp-to-avif-smoke.avif');
  ...
  console.log(`WebP to AVIF browser smoke passed in ${result.elapsed}ms with ${result.size} output bytes, transparent alpha ${result.cornerAlpha}, and zero third-party HTTP(S) requests.`);
} finally {
  await browser.close();
}
