...
try {
  ...
  try {
    ...
    throw new Error(`libheif did not load. moduleState=${JSON.stringify(moduleState)} requests=${JSON.stringify(requestUrls)} console=${JSON.stringify(consoleMessages)}`);
  }
  ...
  assert.equal(build?.trim(), 'Converter build: heic-v12-selfhosted-esm');
